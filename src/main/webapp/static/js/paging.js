	...
	Paging.prototype = {
		init: function(settings) {
			this.settings = $.extend({
                ...
                position: "center",//分页的位置、有left right,center
                hoverBackgroundColor: "#fff",//分页样式鼠标滑过的背景颜色
                hoverColor:"#23527c",//分页样式鼠标滑过的字体颜色
                backgroundColor: "#fff",//分页样式背景颜色
                color:"#ccc",//分页样式字体颜色
			}, settings);
		...
		format: function() {
			if(this.current==0||this.count==0||this.current==""||this.count==""||this.current==null||this.count==null){
				var html='<ul>没有数据</ul>';
				this.container.html(html);
				return;
			}
			var html = '<ul style="text-align: '+this.settings.position+'">'
			...
			$(".ui-paging-container li.ui-pager").css({"background":this.settings.backgroundColor,"color":this.settings.color});
			$(".ui-paging-container li.focus").css({"background":this.settings.hoverBackgroundColor,"color":this.settings.hoverColor});
			var PageThis=this;
			$("li.ui-pager").hover(function(e){
				$(this).css({"background":PageThis.settings.hoverBackgroundColor,"color":PageThis.settings.hoverColor});
			},function(e){
				if(e.target.className.indexOf("focus")>0){

				}else{
					$(this).css({"background":PageThis.settings.backgroundColor,"color":PageThis.settings.color});
				}
		...
		bindToolbar: function() {
			...
			// var PageThis=this;
			// $(".ui-paging-toolbar a").css({"background":this.settings.backgroundColor,"color":this.settings.color});
			// $(".ui-paging-toolbar a").hover(function(e){
			// 	$(this).css({"background":PageThis.settings.hoverBackgroundColor,"color":PageThis.settings.hoverColor});
			// },function(e){
			// 	$(this).css({"background":PageThis.settings.backgroundColor,"color":PageThis.settings.color});
			// })
...
function getNowPageFromPagingJS(){
	var nowPage = $('#Paging').find('ul li.focus').text();
	return nowPage || 1;
}